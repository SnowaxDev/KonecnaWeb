...
### Session 1 (Previous)
- Full-stack SeknuTo.cz website: Homepage, Services, Pricing, Booking, About, Contact
- Multi-step booking form with dynamic price calculation
- Admin panel with login, overview stats, vouchers, coupons, bookings management
- Resend email integration (admin notifications work; customer emails pending domain verification)
- Google Calendar integration (code ready, pending OAuth consent)
- Gallery page with before/after project showcase
- Blog system with CMS admin
- Newsletter popup with coupon delivery
- Security: slowapi rate limiting, atomic DB transactions, MongoDB indexes, protected admin routes
- SEO: react-helmet-async, Schema.org JSON-LD (LocalBusiness, FAQ), sitemap.xml, robots.txt
- Vercel deployment prep: vercel.json, ESLint CI fixes
- Comprehensive DEPLOY.md documentation

### Session 2 (Current - December 2025)
- ✅ Vercel Analytics + Speed Insights activated in App.js JSX
- ✅ Price "od 2 Kč/m²" → "od 2,5 Kč/m²" fixed in PricingPage card + comparison table
- ✅ "✓ Úklid posečené trávy v ceně" info on all mowing services (BookingPage + PricingPage)
- ✅ "extra – více práce (nakládka + odvoz)" info on Odvoz odpadu service (amber color)
- ✅ Global :active click effects on all buttons (scale + brightness via button.jsx cva)
- ✅ Calendar selected day: dark green background + white text + glow ring (calendar.jsx)
- ✅ Admin: Contact Messages tab – full CRUD (GET/PATCH/DELETE), filters, status management
- ✅ Admin: Gallery image upload – POST /api/admin/gallery/upload, /uploads/ static serving
- ✅ Admin: Overview shows "Nových zpráv" StatCard + quick action with badge
...
### Phase 9: SEO Optimalizace (Prosinec 2025) ✅ DONE
- react-helmet-async nainstalován
- SEOHead komponenta s plnou podporou: title, description, keywords, canonical, OG, Twitter Card, JSON-LD
- SCHEMAS helper: localBusiness, faqPage, breadcrumb, service
- Per-page SEO na všech stránkách (HomePage, Services, Pricing, About, Contact, Booking, Admin-noindex)
- LocalBusiness schema s aggregateRating (5/5, 20 recenzí), areaServed (Dvůr Králové, Trutnov, Vrchlabí...)
- FAQ rozšířeno na 6 SEO-relevantních otázek + FAQPage schema
- sitemap.xml vygenerován do /public/
- robots.txt (Disallow: /admin, /api, /poukaz/)
- vercel.json: Security headers (X-Frame-Options, X-XSS-Protection, Referrer-Policy, Permissions-Policy)
- Cache-Control headers pro statické assety (1 rok)
- "Made with Emergent" badge odstraněn z index.html
...
## Changelog – April 2025 (Session 3)
- Gallery upload: base64 into MongoDB (persistent across Render restarts)
- Booking status emails: confirmed/completed/cancelled templates sent to customer
- Contact form auto-reply to customer + improved admin notification template
- Newsletter popup fixed: always returns 200 OK with coupon code
- Admin: Contact Messages tab (CRUD, filters, status)
- Admin: Gallery image upload button (Upload → base64 → MongoDB)
